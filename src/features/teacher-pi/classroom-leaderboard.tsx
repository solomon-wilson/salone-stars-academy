import { Award, Link2, Sparkles } from "lucide-react"
...
export const ClassroomLeaderboard = ({ syncedStudents }: ClassroomLeaderboardProps) => {
  ...
  const handleGenerateInvite = async (pupilId: string) => {
    setGeneratingFor(pupilId)
    setInviteCode(null)
    try {
      const resp = await apiFetch("/api/teacher/generate-invite", {
        method: "POST",
      ...
      console.error(error)
      alert(error instanceof Error ? error.message : "Could not generate invite code.")
    } finally {
  ...
  return (
    <div className="bg-[#0f1233]/60 backdrop-blur-md rounded-3xl p-6 border border-indigo-900/50 shadow-2xl">
      ...
      {inviteCode && (
        <div className="mb-4 flex items-center gap-2 bg-emerald-950/20 border border-emerald-800/40 rounded-xl p-3 text-emerald-300 text-xs">
          <Link2 className="h-4 w-4 shrink-0" aria-hidden="true" />
          <span>
            Parent invite code: <strong className="font-mono tracking-widest">{inviteCode}</strong> — share with the child&apos;s parent.
          </span>
        </div>
      )}
        ...
        {syncedStudents.map((student, i) => (
          ...
          >
            ...
            <div className="flex items-center space-x-3">
              <button
                type="button"
                onClick={() => handleGenerateInvite(student.id)}
                disabled={generatingFor === student.id}
                aria-label={`Generate parent invite for ${student.name}`}
                className="text-[9px] font-black uppercase tracking-wider px-2 py-1 rounded-lg border border-indigo-700/50 text-indigo-300 hover:border-indigo-500 cursor-pointer transition disabled:opacity-50"
              >
                {generatingFor === student.id ? "..." : "Invite"}
              </button>
